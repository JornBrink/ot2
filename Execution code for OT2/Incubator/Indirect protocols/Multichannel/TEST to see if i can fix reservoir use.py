...
mainwd = "C:\\Users\\jornb\\OT2 left\\Development\\Logs"
mainInput = "TryThis.csv"
...
def GetSrcVolume(solutions_map, cmd_line, source_well):
    ...
    #get source amount after aspirated
    src_amt = float(solutions_map[tube_loc[0]][3])
    print(solutions_map[tube_loc[0]][1])
    return src_amt
...
############# MAIN #############
def run(protocol: protocol_api.ProtocolContext, cmdList, deckMap, amtList, pipr, pipl):
 ...
 ############ EXECUTE COMMANDS ############
    ...
    for i in range(len(cmdList)):
        ...
        ########### NOT ANNOTATED ############# / BLOCK1 // TIPRACK EXCHANGE -- 2021/1/5 - Removed by OPENTRONS
        #prompt tip rack reset
        #if(tipID % 96 == 0 and tipID != 1):
        #    input("Please replace tiprack with a full one! >> press ENTER to continue run <<")
        #    left_pipette.reset()
        #    right_pipette.reset()
        ...
        if((pipl == '300m' and (len(target_well)==8 and len(source_well)==8)) or (pipl == '300m' and source_type == 'Reservoir' and len(target_well) == 8)): #(still only when source_well and target well are 8 not when reservoir)
            #operations for multichannel pipette
            pipette = "left_pipette"
            print("=============================")
            print("Multi use on left")
            print('=============================')
        ...
        elif(pipl != "300m" and (len(target_well)!=8 and len(source_well)!=8) or (pipl != '300m' and source_type != 'Reservoir' and len(source_well)!=8)):
            print("=======================")
            print(pipl)
            print("trying to use left not multi")
            print(len(target_well))
            print(len(source_well))
            print('=======================')
        ...
        elif(pipr == '300m' and (len(target_well)==8 and len(source_well)==8) or pipr == '300m' and (source_type == 'Reservoir' and len(target_well) == 8)):
             #(still only when source_well and target well are 8 and not when reservoir) (update 220429 reservoir added)
            #operations for multichannel pipette
            print('=====================')
            print("right is multichannel")
            print('=====================')
