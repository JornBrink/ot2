...
# > Multiplate multichannel (up to 6 plates)
...
shinyServer(function(input, output) {
  #defining directory-------
  outputDir_cmdline <-  "C:\\Users\\jornb\\Desktop\\ot2-main\\MVPlate MCP"
  outputDir_usrGuide <- "C:\\Users\\jornb\\Desktop\\ot2-main\\MVPlate MCP"
  inputTemplate <- "C:\\Users\\jornb\\Desktop\\ot2-main\\MVPlate MCP\\multichannel" 
  sourceDir <- "C:\\Users\\jornb\\Desktop\\ot2-main\\MVPlate MCP\\multichannel\\MVMCsourceFunctions_Jorn.R"
  ...
  contents <- reactive({
    ...
    if(input$do==0){
      ...
      if(errMessage==""){
        ...
        #command line
        cmdLine_name <- paste("CommandList_MC", new_name(), '.csv', sep='')
        write_dir <- paste(outputDir_cmdline, cmdLine_name, sep='/')
        write.csv(new_cmdLineOutput, write_dir, row.names = FALSE)
        
        #user guide
        usrGuide_name <- paste("RobotHandler_MC", new_name(), '.xlsx', sep='')
  ...
  output$downloadTemplate <- downloadHandler(
    filename = "MVMC_InputTemplate.xlsx",
    content = function(file) {
